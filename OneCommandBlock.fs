...
//                sb.Append("Riding:{id:MinecartCommandBlock,Command:\"fill ~ ~-2 ~ ~ ~ ~ air\",") |> ignore 
        ...
        if cmds.[0].StartsWith("MINECART BLOCKS") then
            for i = 1 to cmds.Length-1 do
                ...
                if cmds.[i] = "R" then
                    cmds.[i] <- sprintf "setblock ~ ~%d ~ redstone_block" dy
                elif cmds.[i] = "P" then
                    cmds.[i] <- sprintf "setblock ~ ~%d ~ repeating_command_block" dy
                elif cmds.[i].StartsWith("O ") then
                    cmds.[i] <- sprintf "setblock ~ ~%d ~ command_block 0 replace {Command:\"%s\"}" dy (escape (cmds.[i].Substring(2)))
                elif cmds.[i].StartsWith("C ") then
                    cmds.[i] <- sprintf "setblock ~ ~%d ~ chain_command_block 8 replace {auto:1b,Command:\"%s\"}" dy (escape (cmds.[i].Substring(2)))
                else 
                    cmds.[i] <- sprintf "setblock ~ ~%d ~ chain_command_block 0 replace {auto:1b,Command:\"%s\"}" dy (escape cmds.[i])
